...
function listSensors() {
  ...
  gapi.client.sensors.sensormodel.list().execute(function(resp) {
    console.log(resp);
    ...
    resp.items.forEach(function(sensor) {
      var html = "<tr>";
      ...
      html += "<td><a href='#' onclick='deleteSensor(" + sensor.id + ");'>X</a></td>";
...
function toggleActive(sensorId, active) {
  ...
  gapi.client.sensors.put(request).execute(function(resp) {
      console.log(resp);
      listSensors();
    });
}
...
function deleteSensor(sensorId) {
  var del = gapi.client.sensors['delete']
  del({'id': sensorId}).execute(function(resp) {
    console.log(resp);
    listSensors();
  });
}
...
function createSensor() {
  ...
  gapi.client.sensors.put(sensor).execute(function(resp) {
    console.log(resp);
    listSensors();
  })
}
